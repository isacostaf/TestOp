...
int read_cpu_metrics(cpu_metrics_t* cpu)
{
    ...
    int parsed = sscanf(content, "cpu %lu %lu %lu %lu %lu %lu %lu %lu", &user, &nice, &system, &idle, &iowait, &irq,
                        &softirq, &steal);
...
int run_monitor(int interval_seconds)
{
    ...
    char filename[256];
    snprintf(filename, sizeof(filename), "%s/metrics-%04d%02d%02d.log", METRICS_DIR, tm_info->tm_year + 1900,
        ...
        if (read_system_metrics(&metrics) == 0)
        {
            if (write_metrics_to_file(&metrics, filename) == 0)
            {
                printf("Métricas registradas: CPU=%.1f%%, Mem=%lluKB, Load=%.2f\n", metrics.cpu.cpu_usage,
                       (unsigned long long)metrics.memory.mem_used, metrics.load.load_1m);
            }
            else
            {
